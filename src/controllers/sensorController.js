...
async function getSensorData(req, res) {
  try {
    ...
    let query = `
      SELECT valeur, unite, recorded_at
      FROM sensor_data
      WHERE sensor_id = ?
    `;
    const params = [id];
...
async function getLatestData(req, res) {
  try {
    const { farm_id } = req.query;
    let query = `
      SELECT sd.sensor_id, sd.farm_id, sd.coop_id, sd.type,
             sd.valeur, sd.unite, sd.recorded_at,
             s.modele, s.seuil_min, s.seuil_max
      FROM sensor_data sd
...
async function postSensorData(req, res) {
  try {
    const { sensor_id, farm_id, coop_id, type, valeur, unite } = req.body;

    if (!sensor_id || !farm_id || !coop_id || !type || valeur === undefined) {
      return res.status(400).json({ success: false, message: 'Champs manquants' });
    }

    await pool.query(
      'INSERT INTO sensor_data (sensor_id, farm_id, coop_id, type, valeur, unite) VALUES (?, ?, ?, ?, ?, ?)',
      [sensor_id, farm_id, coop_id, type, valeur, unite || '']
    );
